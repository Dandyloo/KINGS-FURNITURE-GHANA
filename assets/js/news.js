...
(async () => {
  ...
  const buildCard = (article, index) => {
    ...
    return `
      <a href="../pages/article.html?id=${article.id}" class="news-card ${isFeatured ? "news-card--featured" : ""}" data-category="${article.categorySlug}">
  ...
  grid.innerHTML = allArticles
    .map((article, i) => buildCard(article, i))
    .join("");
